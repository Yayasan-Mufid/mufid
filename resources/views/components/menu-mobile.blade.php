...
<div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg md:hidden">
  <div class="flex justify-around">
      ...
      <!-- Data Button -->
      <Link href="{{ route('data.index') }}" class="flex flex-col items-center justify-center p-2 pt-3 {{ request()->routeIs('data') ? 'text-primary-600' : 'text-gray-600' }} hover:bg-gray-100 hover:text-primary-600">
          <x-carbon-data-1 class="w-6 h-6 mb-1 {{ request()->routeIs('data.index') ? 'text-primary-600' : '' }}"/>
          <span class="text-md">Data</span>
      </Link>
      <!-- Informasi Button -->
      <Link href="{{ route('pembayaran.index') }}" class="flex flex-col items-center justify-center p-2 pt-3 {{ request()->routeIs('pembayaran') ? 'text-primary-600' : 'text-gray-600' }} hover:bg-gray-100 hover:text-primary-600">
          <x-carbon-money class="w-6 h-6 mb-1 {{ request()->routeIs('pembayaran.index') ? 'text-primary-600' : '' }}"/>
