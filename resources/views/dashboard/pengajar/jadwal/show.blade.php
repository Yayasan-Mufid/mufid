<x-layouts.dashboard.main :unit="$unit">
    <x-layouts.card>
        ...
        <div class="py-4">
            <div class="mb-4 grid gap-2 sm:grid-cols-2 md:gap-6">
                <div class="flex flex-col">
                    <p class="text-sm font-medium text-gray-700">Pengajar</p>
                    <p class="text-lg font-bold text-gray-800">{{ $jadwal->pengajar->user->name }}</p>
                </div>
                <div class="flex flex-col">
                    <p class="text-sm font-medium text-gray-700">Angkatan</p>
                    <p class="text-lg font-bold text-gray-800">{{ $jadwal->periode->angkatan}}</p>
                </div>
                <div class="flex flex-col">
    ...
    <x-layouts.card>
        <div class="flex items-start justify-center pb-2">
            <form action="{{ route('pengajar.jadwal.absensi', ['unit' => $unit, 'jadwal' => $jadwal->id]) }}" method="GET">
                <div class="flex items-center justify-center">
                    <p>Pilih Absensi :</p>
                </div>
                <div class="flex items-center justify-center">
                    <select name="absenke" class="px-3 py-2 text-base text-gray-700 mr-2 bg-white border border-gray-300 rounded-md shadow-sm appearance-none focus:outline-none focus:ring-primary-500 focus:border-primary-500" required>
                        @for($i = 1; $i <= 16; $i++)
                            <option value="{{ $i }}">Absen Pertemuan {{ $i }}</option>
                        @endfor
                        <option value="ujian">Nilai Ujian</option>
                    </select>
                    <button type="submit" class="text-white bg-primary-500 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 py-2 px-3 rounded">
                        Pilih
                    </button>
                </div>
            </form>
