                        ...
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <span aria-current="page">
                                    <span class="relative inline-flex items-center px-3 py-2 -ml-px text-xs sm:text-sm font-medium  border-primary-500 text-primary-600 z-10 border cursor-default leading-5">{{ $page }}</span>
                                </span>
                            @else
